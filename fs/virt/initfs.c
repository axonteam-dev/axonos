/*
 * Initfs: initrd from Linux boot_params (ramdisk_image/size).
 * Preferred path: SquashFS mount (image stays in place) + overlay upper=ramfs.
...
void initfs_debug_list_vfs(void) {
    qemu_debug_printf("initfs: --- VFS contents after initrd setup ---\n");
    int count = 0;
    initfs_debug_list_vfs_dir("/", 0, &count, 500);
    qemu_debug_printf("initfs: --- VFS total %d entries ---\n", count);
}

/* Mount squashfs image in place (no full-tree copy into ramfs). */
static int initfs_mount_squashfs_region(const void *mod_ptr, size_t mod_size) {
    ...
    klogprintf("initfs: squashfs mounted at / via overlay (image %u bytes, not copied)\n",
               (unsigned)mod_size);
    return 0;
}
...
int initfs_process_linux_bootparams(uint64_t boot_params_phys) {
    ...
    if (rd_sz >= 6) {
        ...
        klogprintf("initfs: ramdisk head %02x %02x %02x %02x %02x %02x "
                   "(squashfs hsqs / newc 070701)\n",
                   h[0], h[1], h[2], h[3], h[4], h[5]);
    }
    const void *mod_ptr = (const void *)rd_pa;
    /* Prefer SquashFS mount (Linux live-root style). Keep cpio unpack as fallback. */
    if (squashfs_image_looks_valid(mod_ptr, rd_sz)) {
        int mr = initfs_mount_squashfs_region(mod_ptr, rd_sz);
        if (mr == 0)
            return 0;
        klogprintf("initfs: squashfs mount failed (%d); not falling back to cpio for hsqs image\n", mr);
        return mr;
    }
