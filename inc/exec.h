...
/* Ensure PID1 has stdin/stdout/stderr on /dev/console before first openat (ld.so). */
...
#define USER_STACK_SIZE USER_STACK_SIZE_LAYOUT
...
#define USER_TLS_SIZE  USER_TLS_SIZE_LAYOUT
#define USER_TLS_BASE  USER_TLS_BASE_LAYOUT
...
int kernel_execve_init_from_path(const char *path, const char *const argv[], const char *const envp[]);
